...
#writer = pd.ExcelWriter("simfin_data4.xlsx", engine='xlsxwriter')

data = OrderedDict()
# get data for each ticker/id

for idx, sim_id in enumerate(sim_ids):
    #os.makedirs(str(tickers[idx]))
    for statement in statement_type:
        d = data[tickers[idx]] = OrderedDict({"Line Item": []})
        if sim_id is not None:
                for year in range(year_start, year_end + 1):
                    for time_period in time_periods:
                        ...
                        request_url = f'https://simfin.com/api/v1/companies/id/{sim_id}/statements/standardised?stype={statement}&fyear={year}&ptype={time_period}&api-key={api_key}'
        ...
        #path = os.makedirs(str(tickers[idx]))
        #path = os.makedirs('data')
        df = pd.DataFrame(d)
        # save in the XLSX file configured earlier
        df.to_csv(str(tickers[idx]) + '_' + str(statement) + ".csv")
        # df.to_csv(os.path.join('data/' + os.makedirs(str(tickers[idx])),
        #                        str(tickers[idx]) + '_' + str(statement) + ".csv"))